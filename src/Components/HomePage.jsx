import { Button } from "@mui/material";
import { useEffect, useState } from "react";
import Header from "../Common/Header";
import one from "../Images/1.png";
import two from "../Images/2.png";
import three from "../Images/3.png";

export default function HomePage() {
  const [toggleDashboard, setToggleDashboard] = useState(false);

  // sample data
  const username = "Pranshu";

  const students = [
    {
      name: "Raghav Matta",
      description:
        "Sophomore @VIT | Full Stack Developer | Ex-Microsoft Intern",
      image:
        "https://writestylesonline.com/wp-content/uploads/2019/01/What-To-Wear-For-Your-Professional-Profile-Picture-or-Headshot.jpg",
      connected: true,
    },
    {
      name: "Yash Bawa",
      description: "Senior @VIT | Android Developer | Google Intern",
      image:
        "https://chicagophotovideo.com/wp-content/uploads/2017/10/ezgif.com-webp-to-jpg-17.jpg",
      connected: true,
    },
    {
      name: "Rohit Bansal",
      description: "Junior @VIT | Backend Developer | Facebook Intern",
      image:
        "data:image/jpeg;base64,",
      connected: false,
    },
    {
      name: "Aryan Signh",
      description: "Freshman @VIT | Blogger",
      image:
        "https://blog-pixomatic.s3.appcnt.com/image/22/01/26/61f166e1377d4/_orig/pixomatic_1572877223091.png",
      connected: false,
    },
    {
      name: "Raghav Matta",
      description: "Sophomore @VIT | Content Creator | Ex-Microsoft Intern",
      image:
        "https://writestylesonline.com/wp-content/uploads/2019/01/What-To-Wear-For-Your-Professional-Profile-Picture-or-Headshot.jpg",
      connected: true,
    },
    {
      name: "Yash Bawa",
      description: "Senior @VIT | Android Developer | Google Intern",
      image:
        "https://chicagophotovideo.com/wp-content/uploads/2017/10/ezgif.com-webp-to-jpg-17.jpg",
      connected: true,
    },
    {
      name: "Rohit Bansal",
      description: "Junior @VIT | Backend Developer | Facebook Intern",
      image:
        "data:image/jpeg;base64,",
      connected: false,
    },
    {
      name: "Aryan Signh",
      description: "Freshman @VIT | Blogger",
      image:
        "https://blog-pixomatic.s3.appcnt.com/image/22/01/26/61f166e1377d4/_orig/pixomatic_1572877223091.png",
      connected: false,
    },
  ];

  useEffect(() => {
    document.title = "Uni Link";
  });

  return (
    <div className="bg-gray-100 flex">
      <div className={`${toggleDashboard ? "absolute" : ""} p-4 md:hidden`}>
        <i
          className={`${
            toggleDashboard ? "fa fa-remove" : "fa fa-bars text-[#0b436e]"
          } relative text-[#66FCF1]`}
          onClick={() => setToggleDashboard(!toggleDashboard)}
        ></i>
      </div>
      <div className="min-h-screen">
        <Header toggleDashboard={toggleDashboard} />
      </div>
      <div className="p-4 w-full text-[#184E77] pt-4">
        <div className="pb-6 flex justify-between">
          <div>
            <p className="text-2xl font-bold">Hi, {username}</p>
          </div>
        </div>
        <div className="flex md:flex-cols-2 gap-4 w-full pt-4">
          <div className="flex flex-col gap-2 justify-center items-center bg-[#ADF5FF] p-6 px-16 w-full md:w-1/4 flex-grow rounded-lg shadow-xl">
            <i className="text-3xl fas fa-users text-[#ADF5FF] bg-[#0075A2] p-2 rounded-full shadow"></i>
            <p className="text-5xl font-bold text-[#0075A2]">{153}</p>
            <p className="text-[#0075A2]">Connections</p>
          </div>
          <div className="flex flex-col gap-2 justify-center items-center bg-[#ADF5FF] p-6 px-16 w-full md:w-1/4 flex-grow rounded-lg shadow-xl">
            <i className="text-3xl fas fa-computer text-[#ADF5FF] bg-[#0075A2] p-2 rounded-full shadow"></i>
            <p className="text-5xl font-bold text-[#0075A2]">{15}</p>
            <p className="text-[#0075A2]">Hackathons</p>
          </div>
          <div className="flex flex-col gap-2 justify-center items-center bg-[#ADF5FF] p-6 px-16 w-full md:w-1/4 flex-grow rounded-lg shadow-xl">
            <i className="text-3xl fas fa-laptop text-[#ADF5FF] bg-[#0075A2] p-2 rounded-full shadow"></i>
            <p className="text-5xl font-bold text-[#0075A2]">{31}</p>
            <p className="text-[#0075A2]">Contests</p>
          </div>
          <div className="flex flex-col gap-2 justify-center items-center bg-[#ADF5FF] p-6 px-16 w-full md:w-1/4 flex-grow rounded-lg shadow-xl">
            <i className="text-3xl fas fa-user-tie px-3 text-[#ADF5FF] bg-[#0075A2] p-2 rounded-full shadow"></i>
            <p className="text-5xl font-bold text-[#0075A2]">{3}</p>
            <p className="text-[#0075A2]">Internships</p>
          </div>
        </div>
        <div className="flex justify-center pt-4">
          <div className="pt-6 flex gap-6 justify-center w-full">
            <Button variant="contained" className="w-1/4" fullWidth>
              Blog
            </Button>
            <Button variant="contained" className="w-1/4" fullWidth>
              Hackathons
            </Button>
            <Button variant="contained" className="w-1/4" fullWidth>
              Contests
            </Button>
            <Button variant="contained" className="w-1/4" fullWidth>
              Internships
            </Button>
          </div>
        </div>
        <div className="pt-6">
          <p className="text-4xl">Leaderboard</p>
          <div>
            <div className="w-full flex flex-col gap-4 pt-4">
              <div className="flex gap-4 bg-blue-200 rounded-lg p-2 relative shadow">
                <div>
                  <img src={one} alt="" width={100} />
                </div>
                <div className="flex items-center">
                  <img
                    src={
                      "https://blog-pixomatic.s3.appcnt.com/image/22/01/26/61f166e1377d4/_orig/pixomatic_1572877223091.png"
                    }
                    alt="profile"
                    className="rounded-full"
                    width={50}
                    height={50}
                  />
                </div>
                <div className="text-xl flex items-center">
                  {"Pranshu Aggarwal"}
                </div>
                <div className="flex items-center">
                  {"Full Stack web developer | App Developer | Sophomore @VIT"}
                </div>
              </div>
              <div className="flex gap-4 bg-blue-200 rounded-lg p-2 relative shadow">
                <div>
                  <img src={two} alt="" width={100} />
                </div>
                <div className="flex items-center">
                  <img
                    src={
                      "https://chicagophotovideo.com/wp-content/uploads/2017/10/ezgif.com-webp-to-jpg-17.jpg"
                    }
                    alt="profile"
                    className="rounded-full"
                    width={50}
                    height={50}
                  />
                </div>
                <div className="text-xl flex items-center">
                  {"Vishal Sharma"}
                </div>
                <div className="flex items-center">
                  {"Full Stack web developer | App Developer | Sophomore @VIT"}
                </div>
              </div>
              <div className="flex gap-4 bg-blue-200 rounded-lg p-2 relative shadow">
                <div>
                  <img src={three} alt="" width={100} />
                </div>
                <div className="flex items-center">
                  <img
                    src={
                      "https://writestylesonline.com/wp-content/uploads/2019/01/What-To-Wear-For-Your-Professional-Profile-Picture-or-Headshot.jpg"
                    }
                    alt="profile"
                    className="rounded-full"
                    width={50}
                    height={50}
                  />
                </div>
                <div className="text-xl flex items-center">
                  {"Radhika Chopra"}
                </div>
                <div className="flex items-center">
                  {"Full Stack web developer | App Developer | Sophomore @VIT"}
                </div>
              </div>

              {students.map((student, index) => (
                <div className="flex gap-4 bg-blue-200 rounded-lg p-2 relative shadow hover:scale-105 transition duration-300">
                  <div>
                    <p className="text-3xl font-bold">{index + 4}.</p>
                  </div>
                  <div className="flex items-center">
                    <img
                      src={student.image}
                      alt="profile"
                      className="rounded-full"
                      width={50}
                      height={50}
                    />
                  </div>
                  <div className="text-xl flex items-center">
                    {student.name}
                  </div>
                  <div className="flex items-center">{student.description}</div>
                  <div className="flex items-center right-4 top-4 absolute">
                    {!student.connected && <i className="fa fa-user-plus"></i>}
                  </div>
                </div>
              ))}
            </div>
          </div>
        </div>
      </div>
    </div>
  );
}
